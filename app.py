...
def init_docker():
    try:
        ...
        # Return None silently - we'll show a friendly message in the UI
        return None
...
if page == "Dashboard":
    ...
    docker = init_docker()
    if not docker:
        st.info("ℹ️ **Running on Streamlit Cloud** — Docker not available")
        st.markdown("""
        This dashboard is designed to manage Docker containers, but Docker is not accessible in this environment.
        
        **Fully working pages:**
        - 🤖 **AI Agent** - Test natural language processing
        - 📊 **Analytics** - View visualizations (with sample data)
        - 🔗 **GitHub API** - Check GitHub status and events
        - ⚙️ **Settings** - Test Groq API connection
        
        **To use with real Docker containers:**
